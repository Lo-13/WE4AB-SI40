    ...
    <form action="rooms" method="GET" class="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
                ...
                <input type="number" step="0.01" min="0" name="min_price" id="min_price" value="<?= htmlspecialchars($minPrice) ?>" placeholder="12"
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="max_price" id="max_price" value="<?= htmlspecialchars($maxPrice) ?>" placeholder="15"
                       class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500">
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <?php if (!empty($rooms)): ?>
            ...
            <div class="bg-gray-900 rounded-xl p-6 border border-gray-800 hover:border-purple-700 transition scroll-reveal tilt-card cursor-pointer">
                ...
                <p class="text-gray-400 text-sm mb-1">Capacite: jusqu'a <?php echo htmlspecialchars($room['capacity']); ?> joueurs</p>
                ...
                <a href="room-detail?id=<?php echo htmlspecialchars($room['id']); ?>" class="block text-center bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg transition text-sm">Voir le detail</a>
            </div>
            <?php
    endforeach; ?>
        <?php
else: ?>
            <p class="text-gray-500">Aucune salle ne correspond a ces filtres.</p>
        <?php
endif; ?>
